...
<template>
    <div class="segmented-control" ref="observerRef">
        <span class="segmented-control-indicator" :style="{
            width: `${width}px`,
            height: `${height}px`,
            transform: `translate(${translate[0]}px, ${translate[1]}px)`
        }"></span>
        <div class="segmented-control-item" v-for="{ value, label } in data" :key="value">
            <input class="segmented-control-input" type="radio" :id="`${id}-${value}`" :name="id" :value="innerValue"
                @input="() => handleChange(value)" />
            <label :for="`${id}-${value}`" :ref="(el) => (refs[value] = el as HTMLLabelElement)"
                :class="['segmented-control-label', { active: innerValue === value }]">{{ label }}</label>
        </div>
    </div>
</template>
