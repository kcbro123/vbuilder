...
const editor = useEditor({
    ...
    content: `
    <p></p>
    <p>我正在使用 Vue.js 运行 Tiptap。 🎉</p>
    <p>您还可以教给编辑器新事物。例如，识别十六进制颜色并添加颜色</p>
    <p>动态颜色：#401f3e #3f2e56、#faf2a1、#759aab、#FD9170、#453f78、#68CEF8、#80cbc4、#9DEF8F</p>
    <p></p>
`
})
</script>
...
<template>
    <div :class="['notes-editor-wrapper', editable && 'editable']">
        <div v-if="editable" class="notes-editor-header">
            <button class="notes-editor-header-button" @click="editor?.chain().focus().toggleBold().run()">
                <text-bold />
            </button>
            <button class="notes-editor-header-button" @click="editor?.chain().focus().toggleItalic().run()">
                <text-italic />
            </button>
            <button class="notes-editor-header-button" @click="editor?.chain().focus().toggleStrike().run()">
                <strikethrough />
            </button>
        </div>
